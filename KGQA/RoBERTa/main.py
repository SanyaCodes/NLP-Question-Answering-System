...
def validate_v2(data_path, device, model, dataloader, entity2idx, model_name, writeCandidatesToFile=False):
    ...
    for i in tqdm(range(len(data))):
        ...
        if pred_ans in ans:
            ...
            num_incorrect += 1
        q_text = d[-1]
        answers.append(q_text[:-2] + '\t' + str(idx2entity[pred_ans]) + '\t' + str(is_correct))
        # except:
        #     error_count += 1
    ...
    writeToFile(answers, '/content/drive/MyDrive/EmbedKGQA_final/output/answers.txt')
...
def train(data_path, neg_batch_size, batch_size, shuffle, num_workers, nb_epochs, embedding_dim, hidden_dim, relation_dim, gpu, use_cuda,patience, freeze, validate_every, hops, lr, entdrop, reldrop, scoredrop, l3_reg, model_name, decay, ls, load_from, outfile, do_batch_norm, valid_data_path=None):
    ...
    print('Model created!')
    if load_from != '':
        # model.load_state_dict(torch.load("/content/drive/MyDrive/EmbedKGQA_final/output/" + load_from + ".pt"))
        fname = "/content/drive/MyDrive/EmbedKGQA_final/output/" + load_from + ".pt"
        # fname = "/scratche/home/apoorv/tut_pytorch/kg-qa//content/drive/MyDrive/EmbedKGQA_final/output/" + load_from + ".pt"
        ...
        for phase in phases:
            if phase == 'train':
                ...
                if score > best_score + eps:
                    ...
                    torch.save(best_model, "/content/drive/MyDrive/EmbedKGQA_final/output/best_score_model.pt")
                    torch.save(best_model, "/content/drive/MyDrive/EmbedKGQA_final/output/" + outfile + ".pt")
                ...
                if epoch == nb_epochs-1:
                    ...
                    torch.save(best_model, "/content/drive/MyDrive/EmbedKGQA_final/output/" + outfile + ".pt")
                    exit()
                # torch.save(model.state_dict(), "/content/drive/MyDrive/EmbedKGQA_final/output/"+str(epoch)+".pt")
                # torch.save(model.state_dict(), "/content/drive/MyDrive/EmbedKGQA_final/output/x.pt")
...
def eval(data_path,
    ...
    if 'half' in hops:
        kg_type = 'half'
    checkpoint_file = '../../pretrained_models/embeddings/commonsense/checkpoint_best.pt'
    print('Loading kg embeddings from', checkpoint_file)
    kge_checkpoint = load_checkpoint(checkpoint_file)
    kge_model = KgeModel.create_from(kge_checkpoint)
    kge_model.eval()
...
data_path = '/content/drive/MyDrive/commonsense_lemma/commonsense300k/QA_train.txt'
valid_data_path = '/content/drive/MyDrive/commonsense_lemma/commonsense300k/QA_val.txt'
test_data_path = '/content/drive/MyDrive/commonsense_lemma/commonsense300k/QA_test.txt'
