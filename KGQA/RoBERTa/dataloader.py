...
class DatasetMetaQA(Dataset):
    def __init__(self, data=None, entities=None, entity2idx=None):
    ...
    def __getitem__(self, index):
        ...
        return question_tokenized, attention_mask, head_id, self.tokenize_question(data_point[2][0])
    ...
    def tokenize_question(self, question):
        ...
        # print()
        # print()
        question_tokenized = self.pad_sequence(question_tokenized, 100)
    ...
    def tokenize_ans(self, ans):
        print(ans)
        ans = "<s> " + ans + " </s>"
        # print('ans')
        # print(ans)
        ans_tokenized = self.tokenizer.tokenize(ans)
        # print('ans_tokenized')
        # print(ans_tokenized)
        # print()
        # print()
        ans_tokenized = self.pad_sequence(ans_tokenized, 100)
        ans_tokenized = torch.tensor(self.tokenizer.encode(ans_tokenized, add_special_tokens=False))
        attention_mask = []
        for q in ans_tokenized:
            # 1 means padding token
            if q == 1:
                attention_mask.append(0)
            else:
                attention_mask.append(1)
        return ans_tokenized, torch.tensor(attention_mask, dtype=torch.long)
